      ...
      Authorization: {
        ...
        custom: {
          options: async (value: string, { req }) => {
            ...
            //1. verify access_token nay xem co phai ciar sever tao ra khong ?
            try {
          ...
          options: async (value: string, { req }) => {
            ...
            try {
              ...
              if (refresh_token === null) {
                throw new ErrorWithStatus({
                  message: USERS_MESSAGES.USED_REFRESH_TOKEN_IS_NOT_EXISTS,
                  status: HTTP_STATUS.UNAUTHORIZED
                })
              }
              ;(req as Request).decoded_refresh_token = decoded_refresh_token
            } catch (error) {
              //neu loi phat sinh trong qua trinh verify thi chuyen thanh loi co status
              ...
              //neu loi k phai dang JsonWebTokenError
              throw error
            }
            return true
          }
  ...
  checkSchema(
    {
      email_verify_token: {
        ...
        custom: {
          options: async (value: string, { req }) => {
            //nếu email_verify_token không gửi lên thì response lỗi
            if (!value) {
              throw new ErrorWithStatus({
